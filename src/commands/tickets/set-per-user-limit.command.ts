...
import { getBotTicketChannelByDiscordId, setBotTicketChannelLimitPerUser } from "../../logic.js";
...
export default defineSubcommand({
	...
	define(builder) {
		return builder
			...
			.addIntegerOption((option) =>
				option.setName("limit").setDescription("Limit per user").setRequired(true).setMinValue(1)
			);
	},
	async execute(interaction) {
		...
		const setLimitResult = await setBotTicketChannelLimitPerUser(ticketChannel, limit);
